...
BasicStream.prototype.feed = function(chars){
    ...
    console.log("feeding", chars.length, "chars");
    for(var i=0;i<chars.length;i++){
        ...
        switch(this.state){
            ...
            case STATES.NORMAL:
                ...
                switch(ch){
                    ...
                    default:
                        ...
                        if(ch <= 0x7f){ //1 byte
                            code = ch
                        }else if(ch>0x7f && ch<=0xE0){ //2 bytes
                            code = ((ch & 0x7f)<<6) + (chars[i+1] & 0x3f) 
                            i++
                        }else if(ch>0xE0 && ch<=0xF0){ //3 bytes
                            code = ((ch & 0xf)<<12) + ((chars[i+1] & 0x3f) << 6) + (chars[i+2] & 0x3f)
                            i+=2
                        }else if(ch>0xE0 && ch<=0xF0){ //3 bytes
                            i+=3
                        }else if(ch>0xF0 && ch<=0xF8){ //4 bytes
                            i+=4
                            console.log("4 bytes");
                        }else if(ch>0xF8 && ch<=0xFC){ //5 bytes
                            i+=5
                            console.log("5 bytes");
                        }else if(ch>0x3FFFFFF && ch<=0x7FFFFFFF){ //6 bytes
                            i+=6
                            console.log("6 bytes");
                        }
                        var drawchar = String.fromCharCode(code)
                        debug("Drawing ", ch); // @debug
                        this.dispatch("draw", drawchar)
                        break;
                }
                break;
            case STATES.ESC:
            ...
            case STATES.CSI:
                ...
                // 'p'
                if( ch === 112 ){
                    if(this.prefix == 33){ // ! soft terminal reset aka DECSTR
                        this.dispatch("softterminalreset", this.params)
                        this.state = STATES.NORMAL;
                        break;
                    }
                }
