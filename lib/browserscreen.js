...
var COLORS_256 = ["000000", "800000", "008000", "808000", "000080",
...
"4e4e4e", "585858", "606060", "666666", "767676", "808080",
...
BrowserScreen.prototype.canvasDisplay = function(){
    ...
    for(var i=0;i<this.screen.rows;i++){
        ...
        for(var j=0;j<this.screen.cols;j++){
            ...
            var outchar = ch.d
            if(outchar){
                if(ch.bg != undefined && ch.bg < COLORS.length && ch.bg != 7){
                    this.context.fillStyle = COLORS[ch.bg]
                    this.context.fillRect((COL_WIDTH*j)+1, ROW_HEIGHT*i, COL_WIDTH+1, ROW_HEIGHT);
                }else{
                    if(ch.rgbbg){
                        this.context.fillStyle = "#" + COLORS_256[ch.rgbbg]
                    }else{
                        this.context.fillStyle = "black"
                    }
                }
                ...
                if(ch.c == undefined){
                    ...
                    this.context.fillStyle = this.foregroundColor;
                    this.curColor = null
                }else if(ch.c != undefined && ch.c != this.curColor){
                    this.curColor = ch.c
                    if(ch.c >= COLORS.length){
                        this.foregroundColor = 'white';
                    }else{
                    ...
                    //console.log("else", ch.c, this.curColor, this.foregroundColor)
