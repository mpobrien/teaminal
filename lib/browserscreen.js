    ...
    for(var i=0;i<this.screen.rows;i++){
        ...
        for(var j=0;j<this.screen.cols;j++){
            ...
            if(outchar){
                if(ch.bg != undefined){
                    if(ch.bg < 0){
                        this.context.fillStyle = "#" + COLORS_256[-ch.bg]
                    }else{
                        this.context.fillStyle = COLORS[ch.bg]
                    }
                    this.context.fillRect((COL_WIDTH*j)+1, ROW_HEIGHT*i, COL_WIDTH, ROW_HEIGHT);
                }else{
                    this.context.fillStyle = "black"
                }
                ...
                if(ch.c == undefined){
                    ...
                    this.curColor = ch.c
                    if(ch.c < 0){
                        this.foregroundColor = '#' + COLORS_256[-ch.c] ;
                    }else if(ch.c >= COLORS.length){
                        this.foregroundColor = 'white';
                    }else{
                ...
                this.context.fillText(outchar, COL_WIDTH * j , ROW_HEIGHT * (i+1))
            }else{
                //
            }
        }
